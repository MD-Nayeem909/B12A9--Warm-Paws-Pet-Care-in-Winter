...
const PetSolution = ({ isVisible = "", pageName = "" }) => {
  ...
  return (
    <div className="mb-16">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full border transition-all duration-200 ${
                  selectedCategory === cat
                    ...
                    : "bg-base-100 border-gray-300 hover:bg-linear-to-r from-indigo-500 to-purple-500 hover:text-white"
                }`}
              >
                {cat}
              </button>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-base-200 p-8">
        {filteredData.length > 0 ? (
          filteredData.map((service, index) => {
            if (pageName && index < 6) {
              return <ServiceCard key={service.serviceId} service={service} />;
            }
            if (!pageName) {
              return <ServiceCard key={service.serviceId} service={service} />;
            }
